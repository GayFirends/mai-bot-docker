...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if len(result_set) > 50:
        await inner_level.finish(f"啊……好多数据。\n（{len(result_set)} 条）\n请老师进行更加详细的说明。")
        return
    s = ""
...
async def _(event: Event, message: Message = EventMessage()):
    ...
    try:
        ...
        if len(music_data) == 0:
            rand_result = "抱歉，老师，我无法定位到您要求的乐谱。"
        else:
            rand_result = song_txt(music_data.random())
        await spec_rand.send(rand_result)
    except Exception as e:
        print(e)
        await spec_rand.finish("抱歉，老师，您输入的指令不正确。")
...
async def _(event: Event, message: Message = EventMessage()):
    ...
    if len(res) == 0:
        await search_music.send("抱歉，老师，无法通过此关键词搜索到乐曲。\n普拉娜的舞萌功能是基于Diving-Fish同志的方案，故没有别名功能，请您谅解。")
    elif len(res) < 50:
        search_result = ""
        ...
        await search_music.send(f"啊……好多数据。\n（{len(res)} 条）\n请老师进行更加详细的说明。")
...
谱师: {chart['charter']}'''
            ...
            await query_chart.send("抱歉，老师，我无法定位到您要求的乐谱。")
    else:
        name = groups[1]
        music = total_list.by_id(name)
        try:
            ...
            await query_chart.send("抱歉，老师，我无法定位到您要求的乐曲。")
...
async def _(event: Event, message: Message = CommandArg()):
    ...
    for i in range(11):
        if wm_value[i] == 3:
            ...
            s += f'忌 {wm_list[i]}\n'
    s += "老师，虽然每天的出勤很快乐，但也请您注意对机台的保护，不要大力拍打或进行危险的动作。\n今日推荐歌曲："
...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if success == 400:
        await best_40_pic.send("抱歉，老师，我无法定位到您输入的玩家名字。")
    elif success == 403:
...
async def _(event: Event, message: Message = CommandArg()):
    ...
    if success == 400:
        await best_50_pic.send("抱歉，老师，我无法定位到您输入的玩家名字。")
    elif success == 403:
